...
def get_applicable_rules(string1, all_rules):
    #for applicable rules
    rules_prime = set()

    for rule in all_rules:
        if rule[0] in string1:
            if ' ' in rule[1]:
                temp = rule[1].split(' ')
                for j in temp: rules_prime.add(j)
            else:
...
def full_expansion(string1, string2, all_rules):
    ...
    rules_prime_1 = get_applicable_rules(string1, all_rules)
    rules_prime_2 = get_applicable_rules(string2, all_rules)
    # print(rules_prime_1)
    # print(rules_prime_2)
